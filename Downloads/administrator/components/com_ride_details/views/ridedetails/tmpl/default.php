...
$document->addStyleSheet(JUri::root() . 'administrator/components/com_ride_details/assets/css/ride_details.css');
$document->addStyleSheet(JUri::root() . 'media/com_ride_details/css/list.css');

$user      = JFactory::getUser();
$userId    = $user->get('id');
$listOrder = $this->state->get('list.ordering');
$listDirn  = $this->state->get('list.direction');
$canOrder  = $user->authorise('core.edit.state', 'com_ride_details');
$saveOrder = $listOrder == 'a.`ordering`';

if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_ride_details&task=ridedetails.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'ridedetailList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
}
	...
	<?php echo JHtml::_('grid.sort', 'JSTATUS', 'a.`state`', $listDirn, $listOrder); ?>
				...
				<?php foreach ($this->items as $i => $item) :
					...
					<tr class="row<?php echo $i % 2; ?>">
						...
						<?php if (isset($this->items[0]->state)): ?>
							<td class="center">
	<?php echo JHtml::_('jgrid.published', $item->state, $i, 'ridedetails.', $canChange, 'cb'); ?>
</td>
						<?php endif; ?>
				...
				<?php endif; ?>
				<?php if ($canEdit) : ?>
					<a href="<?php echo JRoute::_('index.php?option=com_ride_details&task=ridedetail.edit&id='.(int) $item->id); ?>">

...
if (empty($list['direction']))
{
	...
	protected function getListQuery()
	{
		...
		if (!JFactory::getUser()->authorise('core.edit', 'com_driver_details'))
		{
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('( a.dname LIKE '.$search.'  OR  a.demail LIKE '.$search.'  OR  a.dmobile LIKE '.$search.'  OR  a.cab_no LIKE '.$search.'  OR  a.cab_type LIKE '.$search.' OR  a.route_prefer LIKE '.$search.' )');
			}
		}
	...
	public function getItems()
	{
		...
		foreach ($items as $item)
		{
					...
					$item->dgen = JText::_('COM_DRIVER_DETAILS_DRIVERDETAILS_DGEN_OPTION_' . strtoupper($item->dgen));
		}
	...
	protected function loadFormData()
	{
		...
		if ($error_dateformat)
		{
			$app->enqueueMessage(JText::_("COM_DRIVER_DETAILS_SEARCH_FILTER_DATE_FORMAT"), "warning");

 ...
 */
class jetestimonialViewCategory extends JViewLegacy
{
	protected $state;
	protected $items;
	protected $category;
	...
	function display($tpl = null)
	{
		...
		// Get some data from the models
			...
			if (empty($this->item->id)) {
				$authorised							= $user->authorise('core.create', 'com_jetestimonial') || (count($user->getAuthorisedCategories('com_jetestimonial', 'core.create')));
			} else {
		...
		// Check whether category access level allows access.
			...
			if( $cat_id > 0 && $cat_view == 'category' ) {
				$values						= $this->get('testimonialcategories');
				if( count($values) > 0 ) {
		...
		$testimonial_model					= JModelLegacy::getInstance('Testimonials', 'jetestimonialModel');
		...
		$this->assignRef('model',		$testimonial_model);
	...
	protected function _prepareDocument()
	{
		...
		if ($menu) {
			...
			$this->params->def('page_heading', JText::_('COM_JETESTIMONIAL_DEFAULT_PAGE_TITLE'));
		}
		...
		if ($menu && ($menu->query['option'] != 'com_jegaqpro' || $menu->query['view'] == 'faqs' || $id != $this->category->id)) {
			...
			while (($menu->query['option'] != 'com_jetestimonial' || $menu->query['view'] == 'faqs' || $id != $category->id) && $category->id > 1) {
				$path[]						= array('title' => $category->title, 'link' => jetestimonialHelperRoute::getCategoryRoute($category->id));

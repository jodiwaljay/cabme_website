...
class UsersModelReset extends JModelForm
{
	...
	public function getResetChangePasswordForm($data = array(), $loadData = true)
	{
		// Get the form.
		$form = $this->loadForm('com_users.default_changepassword', 'default_changepassword', $options = array('control' => 'jform'));
	...
	//Password change function 25-02-2016
	/**
	 ...
	 * @since   1.6
	 */
	public function processResetCheck($data)
	{
		$user =& JFactory::getUser();
		$userId = $user->get( 'id' );

		if(empty($data['currentpass']) && empty($userId))
		{
			return false;
			...
			if($user){
				$match = JUserHelper::verifyPassword($data['currentpass'], $user->password, $user->id);
				if($match == true){
				...
				/*--change password---*/
					...
					if ($user->requireReset == 1 && JUserHelper::verifyPassword($data['currentpass'], $user->password))
					{
					...
					$this->setError('Please Enter Valid Current Password');

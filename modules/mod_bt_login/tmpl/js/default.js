...
BTLJ(document).ready(function() {
	...
	BTLJ('.already_login a').click(function (event) {
			if(BTLJ('.btl-modal').length){
				BTLJ.modal.close();setTimeout("showLoginForm();",1000);
			}
			else{
				showRegistrationForm();
			}
...
function dshowLoginForm(obj){
	window.tempclick = obj;
...
function registerAjax(){
	...
	if(BTLJ("#btl-input-profile-pic").val()==""){
	BTLJ("#btl-registration-error").html(Joomla.JText._('REQUIRED_PROFILEPIC')).show();
	BTLJ("#btl-input-profile-pic").focus();
	return false;
	}
	...
	if(BTLJ("#btl-input-tel").val()==""){
		BTLJ("#btl-registration-error").html(Joomla.JText._('REQUIRED_TEL')).show();
		BTLJ("#btl-input-tel").focus();
		return false;
	}
	var telRegExp = /^\d{10}$/;
		if(!telRegExp.test(BTLJ("#btl-input-tel").val())){
			BTLJ("#btl-registration-error").html(Joomla.JText._('TEL_INVALID')).show();
			BTLJ("#btl-input-tel").focus().select();
			return false;
		}
	...
	var datasubmit= "bttask=register&name="+encodeURIComponent(BTLJ("#btl-input-name").val())
			...
			+"&mobile=" + encodeURIComponent(BTLJ("#btl-input-tel").val())
			+"&postpicname=" + encodeURIComponent(BTLJ("#postpicname").val())
			+ "&"+token+"="+value_token;
	if(btlOpt.RECAPTCHA =="recaptcha"){
...
function loginAjax(){
	...
	BTLJ.ajax({
	   ...
	   success: function (html, textstatus, xhrReq){
		  if(html == "1" || html == 1){
			   if(window.tempclick == undefined)
					window.location.href=btlOpt.BT_RETURN;
				else{
					window.checkLogin = false;
					BTLJ(".simplemodal-close").click();
					jQuery(window.tempclick).click();
				}
		   }else{
			   if(html.indexOf('</head>')==-1){
				...
				else
				{
					if(html.indexOf('btl-panel-profile')==-1){
						showLoginError('Username and password do not match or you do not have an account yet.');
					}
					else
					{
						if(window.tempclick == undefined)
							...
							window.checkLogin = false;
							BTLJ(".simplemodal-close").click();
							jQuery(window.tempclick).submit();
						}
					}
				}
		   }

...
BTLJ(document).ready(function() {
	...
	BTLJ('#register-link a').click(function (event) {
		if(BTLJ('.btl-modal').length){
			...
			setTimeout(showRegistrationForm(),9000);
		}
		else{
			return showRegistrationForm();
		}
	...
	BTLJ(document).click(function(event){
		if(requireRemove && event.which == 1) btTimeOut = setTimeout('BTLJ("#btl-content > div").slideUp();BTLJ(".btl-panel span").removeClass("active");',50);
		requireRemove =true;
	})
